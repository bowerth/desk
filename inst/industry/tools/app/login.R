...
hash <- digest(paste0(salt, "eas"), algo="sha256")

userTable <<- data.frame(user        = "eas",
                         login       = hash,
                         nacl        = salt)
...
loginUI <- function (){
  div(class= "",
      ...
      br(),
      br(),
      shinysky::actionButton("login", "Login", styleclass="success",icon = NULL, size = "small", block = FALSE), # block = TRUE
      br(),
      br(),
      br(),
      br()
