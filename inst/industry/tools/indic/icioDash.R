...
##
ui.icioDash.dim_conv <- list(row=c(ui.icioDash.nocou, ui.icioDash.noind), col=c(ui.icioDash.nocou))
...
icioDash <- function(
  ...
  ## iciodash_couXlist <- c("USA", "AUT", "AUS", "CAN", "FRA", "DEU")
  ...
  data <- dat$DATA.ICIO6234GRTR[match(iciodash_year, ui.icioDash.year),,]
  ...
  data.out <- aggMatrix_icioDash(data = data,
                                 toRow = toRow,
                                 toCol = toCol,
                                 displayConv = FALSE)
