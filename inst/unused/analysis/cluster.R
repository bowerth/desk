...
hc_dist_method <- c("Squared euclidean" = "sq.euclidian",
                    "Euclidian" = "euclidean",
                    "Maximum" = "maximum",
                    "Manhattan" = "manhattan",
                    "Canberra" = "canberra",
            ...
            conditionalPanel(condition = "input.km_hcinit == true",
                             wellPanel(
                                 selectInput("km_dist",
                                             ...
                                             choices = hc_dist_method,
                                             selected = state_init_list("km_dist","sq.euclidian", hc_dist_method),
                                             multiple = FALSE),
                                 selectInput("km_meth",
                                             label = "Method:",
                                             choices = hc_method,
                                             selected = state_init_list("km_meth","ward", hc_method),
                                             multiple = FALSE)
                                 )
                             ),
...
summary_kmeansCluster <- function(result = .kmeansCluster())
{
    ...
    cat(paste0("\nBetween cluster variance accounts for ",
               sprintf("%.2f",perc_between),
...
plots_kmeansCluster <- function(result = .kmeansCluster())
{
    ...
    {
        plots[[var]] <- ggplot(dat, aes_string(x=var, fill='clusvar')) + geom_density(adjust=1.5, alpha=.3) + labs(y = "") + theme(axis.text.y = element_blank())
    }
