...
output$ui_Manage <- renderUI({
  list(
    ...
    ##   radioButtons(
    ##     inputId = "dataType",
    ##     label = "Load data:", c(".rda" = "rda", ".csv" = "csv", "clipboard" = "clipboard", "examples" = "examples"),
    ##     selected = "rda"),
...
output$htmlDataExample <- renderPrint({
...
##   names(dat)
  ...
  ## obj <- "DATA.ANBERD"
  ## obj <- input$dataframes
  ## for (obj in names(dat))
  ## {

  str(dat)

      ## nr <- min(4,nrow(dat[[obj]]))
      ## obj.df <- data.frame(dat[[obj]][1:nr,, drop = FALSE])
      ## obj.names <- names(obj.df)
      ## obj.df$obj <- obj
      ## obj.df <- subset(obj.df, select = c("obj", obj.names))
      ## obj.df <- date2character_dat(obj.df)

      ## html <- print(xtable::xtable(obj.df), type='html', print.results = FALSE)

      ## html <- sub("<TABLE border=1>","<table class='table table-condensed table-hover'>", html)
      ## ## html <- sub("<TABLE border=1>",paste0("<table class='table table-condensed table-hover'>\n<TR> test </TR>"), html)
